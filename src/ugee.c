...
/* Initialize data */
    ...
    *maxiter = *S_iter;
    ...
    // alpha_VC_GEE_bandwidth: bandwidth for the covariance matrix.
    // defined as Mv + 1.
...
/* xpx = VC_GEE_create_matrix(*p, *p, EPHEMERAL);
   xpy = VC_GEE_create_matrix(*p, one, EPHEMERAL); */
	...
	for (i = 0 ; i < nclust ; i++)
	{
	  // ni: number of observations for cluster i.
	    ni = Y[i]->nrows;
	  // dni: same as ni as double type.
	    dni = (double)ni;
...
//		if ((maxfitted >= .999999) && var_mean_rel == (int) Binomial)
	    ...
	    // for Toeplitz or AR structures:
	    if (corstruct == (int) stat_M_dep || corstruct == (int) AR_M)
	    {
	      // Assertion: We need to have at least the bandwith number observations
	      // for each cluster. Otherwise fail.
	      // DSB: let's comment this and see where the current code breaks.
	      // interesting we get infinite loop
	      ...
	      // Initialize a permanent row vector of bandwidth length.
	      lag_wts = VC_GEE_create_matrix(1, alpha_VC_GEE_bandwidth, PERMANENT);

	      // First element of lag_wts is set to 1.
	      MEL(lag_wts, 0, 0) = (double)1.;

	      // Iterate through remaining elements i = 1, ..., M
	      // and set elements to N / (N - i).
	      // for (ini = 1 ; ini < alpha_VC_GEE_bandwidth ; ini++)
	      // {
	      //   MEL(lag_wts, 0, ini) = dni/(dni - (double)ini);
	      // }
	      // because dni = 3 and ini also goes until 3 we got before
	      // lag_wts = {1, 1.5, 3, inf}
	    ...
	    case exchangeable:
		 ...
		 is redundant */
		break;
		// In the Toeplitz and AR case, we modify alpha as:
		// alpha += (lag_wts^T * deltaT)^T = delta * lag_wts
		// where delta = "lag matrix for e_i"
		// where e_i is a "residual" vector for cluster i which has been defined earlier.
	    case stat_M_dep:
	    case AR_M:
// lag_wts already has an Inf in last element here
	      alpha = VC_GEE_matadd(
	        alpha,
	        VC_GEE_transp(
	          VC_GEE_px1_times_pxq (
	              VC_GEE_transp(lag_wts),
	              VC_GEE_transp(VC_GEE_covlag(ei, alpha_VC_GEE_bandwidth, 0))
	          )
	        )
	      );
	      break;
	...
	// already here alpha is not right (nan at the end)
	...
	case stat_M_dep:
	  ...
	  // alpha is not right here already.
	  // alpha is plugged into tmpR and then destroyed afterwards.
	  VC_GEE_plug(alpha, tmpR, 0, 0);
	  MEL(tmpR, 0, 0) = (double)1.;
	  // tmpR already does not look right here, nan at the end.

	  // R is the Toeplitz (correlation?) matrix for all clusters and observations.
	  // So we would have to modify this...
	  // Or maybe we can just subset this matrix after all this?
	...
	for (i = 0 ; i < nclust ; i++)
	{
	  ...
	  {
	    // DSB: or we can even just do it here? ...
	    // instead of just requesting the top left corner of R
...
// Allocates an nrows x ncols matrix of zeros and returns the pointer.
